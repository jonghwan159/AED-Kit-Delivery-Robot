    ...
    void poseCallback(const gps_imu_ekf::Gnss::ConstPtr& msg) {
        ...
        if (Astar_done == false) {
            ...
            for (size_t i = 1; i < global_path_.size(); ++i) {
                total_path_length += getDistance(global_path_[i-1], global_path_[i]);
            }
            o+=duration;
            cout << "Astar done" << endl;
            cout << "Total path length: " << total_path_length << " units" << endl;
            cout << "Path planning time: " << o << " ms" << endl;
            Astar_done = true;
        }
